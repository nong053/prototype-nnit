		...
		if($num_rows=mysql_num_rows($table)) {
			...
			while($row=mysql_fetch_array($table)) {
				if($row['admin_status'] == '1'){
					$admin_status1 = "<div id=\"status\" style=\"color:#006600\">ใช้งานได้</div>\r\n";
				}elseif($row['admin_status'] == '3'){
					$admin_status1 = "<div id=\"status\" style=\"color:#006600\">Supper User</div>\r\n";
				}else{
					$admin_status1 = "<div id=\"status\" style=\"color:#ff0000\">ระงับชั่วคราว</div>\r\n";
				}
				...
				if(strlen($row['admin_website'])>15){
					$admin_website=mb_substr($row['admin_website'],1,15,"UTF-8")."...";
					
				}else{
					$admin_website=$row['admin_website'];
				}
				
				echo "<div id=\"sname\"><a href=\"admin_detail.php?admin_id=".$row['admin_id']."&TB_iframe=true&height=200&width=500\" rel=\"sexylightbox\" title=\"".$row['admin_name']." ".$row['admin_surname']."\">".$admin_website."<br></a></div>\r\n";
				
				
				
				echo "<div id=\"status\">".$admin_status1."</div>\r\n";
	...
	if($_GET['action'] == "edit"){
		...
		$table=mysql_query($sql) or die(mysql_error());
			if($row=mysql_fetch_array($table)) {
				...
				$admin_status1 = $row['admin_status'];
				$admin_email = $row['admin_email'];
				$admin_website = $row['admin_website'];
				$admin_send_email = $row['admin_send_email'];
...
<div id="frm-admin2">
  <input name="admin_name" type="text" class="frm-text" value="<?=$admin_name1?>" />
</div>
<br style="clear:both"  />
</div>
...
<div id="frm-admin">สถานะ</div>
	<div id="frm-admin2">
	<? if( $_SESSION['admin_status'] == '3' && $admin_status1 != '3' ){ ?>
		<select name="admin_status" id="admin_status" style="border:#dedede solid 1px;">
		  <option <? if($admin_status1 == "1"){echo "selected=\"selected\"";}?> value="1">ใช้งานได้</option>
		  <option <? if($admin_status1 == "0"){echo "selected=\"selected\"";}?> value="0">ระงับชั่วคราว</option>
  	    </select>
	<? }else{ ?>
		...
		<input name="admin_status" id="admin_status" type="hidden" value="<?=$admin_status1?>">
	<? } ?>
