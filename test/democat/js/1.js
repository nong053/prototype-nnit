...
$(function(){
  // policylike
  $('#likepolicy').click(function(){
    if($(this).hasClass('active')){}else{
      var title = $('#title h2').text()
      _gaq.push(['_trackEvent', 'Policies', 'Like', title]);
  ...
  if($('body').hasClass('front')){
    $.getJSON('/popupbox.json',function(data){
      var item = data.nodes[0].node;
      if(item.type == 'video'){
        item.youtube += '&autoplay=1';
        $.fancybox({
        'padding'		: 0,
        'autoScale'		: false,
        'transitionIn'	: 'none',
        ...
        'width'		: 680,
        'height'		: 495,
        'href'			: item.youtube.replace(new RegExp("watch\\?v=", "i"), 'v/'),
        'type'			: 'swf',
        'swf'			: {
          'wmode'		: 'transparent',
  ...
  splashTimer = setInterval('rotateSplash()',5700);
  //var totalBanner = $('#featured .item').length;
  $('<ul/>').attr('id','splash-nav').html('<li rel="1" class="active">1</li><li rel="2">2</li><li rel="3">3</li><li rel="4">4</li><li rel="5">5</li><li rel="6">6</li>').appendTo($('#featured'));
  	$('#splash-nav li').click(function(){
    	gotoSplash($(this).attr('rel'));
  	});
  	
  $('#featured .featured-item:eq(0)').show();
  	
  $("a.button-qr").fancybox({
    width:300,height:300
  });
  
  $(".views-field-field-link-video-embed a, a.youtube").click(function() {
  ...
  $(".views-field-field-flickr-set-embed a, a.flickr_sets").click(function() {
  var set = this.href.replace(new RegExp("(.*)/sets/",""),'');
  	$.fancybox({
  			'padding'		: 0,
  			'autoScale'		: false,
  			'transitionIn'	: 'none',
  			...
  			'width'		: 680,
  			'height'		: 455,
  			'href'			: 'http://www.flickr.com/apps/slideshow/show.swf?v=71649&offsite=true&lang=en-us&set_id='+set+'&page_show_url='+this.href.replace("http://www.flickr.com",'')+'/show',
  			'type'			: 'swf',
  			'swf'			: {
  			   	 'wmode'		: 'transparent',
  				'allowfullscreen'	: 'true'
...
function rotateSplash(){
  //if($.browser.msie) $('#splash-'+currentSplash).hide(); else $('#splash-'+currentSplash).fadeOut(400);  
  var to = parseInt(currentSplash)+1;
  if(to > maxSplash) to = 1;
  gotoSplash(to);
  
  $('#splash-nav li.active').removeClass('active');
  $('#splash-nav li[rel='+to+']').addClass('active');
  //if($.browser.msie) $('#splash-'+currentSplash).show(); else $('#splash-'+currentSplash).fadeIn(400);
}

function gotoSplash(to){
  clearInterval(splashTimer);
  splashTimer = setInterval('rotateSplash()',5000);
  swapSplash(to);
  $('#splash-nav li.active').removeClass('active');
  $('#splash-nav li[rel='+currentSplash+']').addClass('active');
}

function swapSplash(to){
  to = parseInt(to);
  $('#featured .featured-item:eq('+(currentSplash-1)+')').fadeOut(400);
  $('#featured .featured-item:eq('+(to-1)+')').fadeIn(400);
  currentSplash = to;
}
...
function MobileSwitch(){
  ...
  { 
    ...
    var urlTofwd = "http://mobile.democrat.or.th" + location.pathname;
