...
bgColorChanger.blankImgSrc = 'blank.gif';
...
htmlStr += getTableTag(0,3,0,'100%') + '<tr><td><font color="#FFFFFF" face="verdana,arial" size="1"> Select a new Background Color</font></td>';
...
function openBgColorChanger() {
if (dyn) {
if (!bgColorChanger.isLoaded) loadBgColorChanger();
var posX = (getWinWidth() - bgColorChanger.boxWidth) / 2;
var posY = (getWinHeight() - bgColorChanger.boxHeight) / 2;
moveLayerTo(bgColorChanger.layerObj,getDocScrollLeft() + posX,getDocScrollTop() + posY);
...
<form>
<input type="button" value="เลือกสีพื้นหลัง" onClick="javascript:openBgColorChanger();">
</form>
