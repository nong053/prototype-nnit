<?php 
	...
	$main_menu_type= trim($_POST['main_menu_type']);
	...
	if(!$product_name){
	echo"<script>alert(\"ใส่ชื่อเมนูด้วยครับ\");</script>";
	echo"<script>window.location=\"index.php?page=menu_system&select_page=main_menu&productcat_id=".$productcat_id."\"</script>";
	exit();
	}
...
$path_thumbnail="../menu_pic/".$productcat_id."/".$product_id."/thumbnail/";
$path_big_picture="../menu_pic/".$productcat_id."/".$product_id."/";
	...
	rmdir("$path_big_picture");
		...
		$sql="UPDATE menu SET product_name='$product_name',product_title='$product_title', product_detail='$product_detail',product_price='$product_price',main_menu_type='$main_menu_type',main_menu_link='$main_menu_link',product_promotion='$product_promotion',product_amount='$product_amount' WHERE product_id='$product_id'";
		mysql_query($sql)or die (mysql_error());
		
		$product_path = "../menu_pic/" . $productcat_id . "/" . $product_id . "/";
		...
		if($row=mysql_fetch_array($table)) {
			?>
			<script>
			alert("ชื่อเมนูนี้มีอยู่ในระบบแล้ว !");
			window.history.back();
			</script>
			<?
			exit();
		...
		$sql="INSERT INTO menu (productcat_id, product_name,product_title, product_detail,product_price,main_menu_type,main_menu_link,product_promotion,product_amount,product_date) VALUES ('$productcat_id', '$product_name','$product_title', '$product_detail','$product_price','$main_menu_type','$main_menu_link','$product_promotion','$product_amount',now())";
		mysql_query($sql)or die (mysql_error());
		...
		header("Location: index.php?page=menu_system&select_page=main_menu&productcat_id=".$productcat_id."");
	}
	header("Location: index.php?page=menu_system&select_page=main_menu&productcat_id=".$productcat_id."");
